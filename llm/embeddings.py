...
def crear_embeddings(
    ...
    (default ``"ollama"``). Se puede sobreescribir con el parámetro
    ``proveedor``.

    Parámetros:
        proveedor: Nombre del proveedor. ``"ollama"`` | ``"openai"``.
            Si es ``None``, se lee de ``EMBEDDING_PROVIDER`` (default ``"ollama"``).
        modelo: Nombre del modelo a usar. Si es ``None``, se usa el default
            de cada proveedor.
    ...
    """
    proveedor = proveedor or os.getenv("EMBEDDING_PROVIDER", "ollama")
